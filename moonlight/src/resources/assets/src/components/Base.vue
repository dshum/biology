  ...
  <div>
    <nav @click="closeMenu()">
      <div class="container">
        ...
        <ul class="right">
          <li v-if="loggedUser && avatar">
            <div class="avatar" @click.stop="toggleMenu()">
              <img :src="avatar" /><i class="fa fa-caret-down" aria-hidden="true"></i>
            </div>
          </li>
        </ul>
        <ul class="switch">
    ...
    <div @click="closeMenu()">
        ...
        <div class="container">
          ...
          <transition name="switch">
            <div v-show="mode === 'users'">
              <ul class="menu">
                <li><router-link to="/groups"><i class="fa fa-users"></i>Группы</router-link></li>
                <li><router-link to="/users"><i class="fa fa-user"></i>Пользователи</router-link></li>
                <li><router-link to="/log"><i class="fa fa-clock-o"></i>Журнал</router-link></li>
                <li><router-link to="/profile"><i class="fa fa-pencil"></i>Редактировать профиль</router-link></li>
                <li><router-link to="/password"><i class="fa fa-lock"></i>Сменить пароль</router-link></li>
                <li @click="logout()"><span><i class="fa fa-sign-out"></i>Выход</span></li>
              </ul>
            </div>
          </transition>
        </div>
      </div>
      <div class="context" @click="closeContextMenu()">
        <ul v-if="contextMenuElement">
          <li class="title">{{ contextMenuElement.name }}<br /><small>{{ contextMenuItem.name }}</small></li>
          <li><router-link :to="{name: 'browse', params: {classId: contextMenuElement.classId}, query: {mode: 'edit'}}">Редактировать</router-link></li>
          <li><router-link :to="{name: 'browse', params: {classId: contextMenuElement.classId}}">Открыть</router-link></li>
          <li class="divider"></li>
          <li @click="confirmDelete = true"><span>Удалить</span></li>
        </ul>
      </div>
      <transition name="fade">
      ...
      <transition name="fade">  
        <confirm v-if="contextMenuElement" v-show="confirmDelete" confirmButton="Удалить" confirm-button-class="remove" v-on:confirm="deleteElement()" v-on:cancel="confirmDelete = false">
          Удалить элемент &laquo;{{ contextMenuElement.name }}&raquo;?
        </confirm>
      </transition>
      <transition name="fade">
...
export default {
  ...
  methods: {
    ...
    onContextMenu (event, item, element) {
      ...
      if (top + $('context').height() > $(window).height()) {
        if (top - $('context').height() < 0) {
          top = 10
        } else {
          top = top - ($('context').height() + 10)
        }
      } else {
        top = top + 10
      }
    ...
    deleteElement () {
      ...
      this.$http.delete('/edit/' + this.contextMenuElement.classId).then((response) => {
        let data = response.body
